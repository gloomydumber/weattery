...
contract WeatherOracle is FunctionsClient {
    ...
    string public source = "const city = args[0];" "const apiResponse = await Functions.makeHttpRequest({"
        "url: `https://wttr.in/${city}?format=3&m`," "responseType: 'text'" "});" "if (apiResponse.error) {"
        "throw Error('Request failed');" "}" "const { data } = apiResponse;" "return Functions.encodeString(data);";
    ...
    function getTemperature(string memory _city) external returns (bytes32 requestId) {
        ...
        CityStruct memory auxCityStruct = CityStruct({sender: msg.sender, timestamp: 0, name: _city, temperature: ""});
    ...
    function listCities(uint256 start, uint256 end) public view returns (CityStruct[] memory) {
        if (end > cities.length) {
            end = cities.length - 1;
        }
        require(start <= end, "start must <= end");
        uint256 cityCount = end - start + 1;
